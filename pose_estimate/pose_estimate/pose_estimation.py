...
class ImageSubscriber(Node) :
    def __init__(self) :
        ...
        # subscriber의 타입 지정
        self.subscription = self.create_subscription(Image, '/rgb_left', self.listener_callback, qos_profile_sensor_data)
        # Image : topic에 사용되는 메세지 타입
        # /rgb_left : subscribe할 토픽 이름
        # 10 : 통신할 때, 데이터를 바로 보낼 수 없다면 10개까지 모아둠
    ...
    def listener_callback(self, msg) :
        ...
        frame_provider = cv_img
        is_video = False
        base_height = 256
        fx = -1 # 카메라 focal length
        ...
        mean_time = 0
        stride = 8
        net = InferenceEnginePyTorch("human-pose-estimation-3d.pth", 'GPU', True)
...
class PosePublisher(Node) :
    def __init__(self) :
        ...
        self.publisher = self.create_publisher(Image, "/image_with_pose", 10)
...
def main() :
    ...
    args.model = "human-pose-estimation-3d.pth"
