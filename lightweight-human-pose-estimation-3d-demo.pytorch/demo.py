...
#         poses_3d, poses_2d = parse_poses(inference_result, input_scale, stride, fx, is_video) # pose_3d는 world coordinate 기준
#         edges = []
#         if len(poses_3d):
#             poses_3d = rotate_poses(poses_3d, R, t) # rotate_poses로 인해 poses_3d가 camera coordinate 기준으로 바뀜
...
#             poses_3d = poses_3d.reshape(poses_3d.shape[0], 19, -1)[:, :, 0:3] # 19개의 body point들의 좌표(camera coordinate) : 코;1 눈(좌/우);16/15 귀(좌/우);18,17
...
class trajPub(Node):
    def __init__(self):
        super().__init__('trajPub')
        qos_profile = QoSProfile(depth=10)
        self.publisher_ = self.create_publisher(Twist, 'traj', qos_profile)
        self.msg = Twist()
...
if __name__ == '__main__':
    ...
    for frame in frame_provider:
        ...
        if len(poses_3d):
            ...
            edges = (Plotter3d.SKELETON_EDGES + 19 * np.arange(poses_3d.shape[0]).reshape((-1, 1, 1))).reshape((-1, 2))
            #
            traj.msg.linear.x = float(poses_3d[0,1,0])
            traj.msg.linear.y = float(poses_3d[0,1,1])
            traj.msg.linear.z = float(poses_3d[0,1,2])
            traj.publish()
            #
        plotter.plot(canvas_3d, poses_3d, edges)
        cv2.imshow(canvas_3d_window_name, canvas_3d)
